...
-- Adicionando constraint check no status da tabela video
ALTER TABLE MC_VIDEO
    ADD CONSTRAINT CK_MC_VIDEO_ST_VIDEO CHECK (ST_VIDEO IN ('A', 'I'));

-- Adicionando constraint check no status do produto
ALTER TABLE MC_PRODUTO
    ADD CONSTRAINT CK_MC_PRODUTO_ST_PRODUTO CHECK (ST_PRODUTO IN ('A', 'I'));
    
-- Adicionando Unique constraint na descricao da categoria do produto
ALTER TABLE MC_CATEGORIA_PROD
    ADD CONSTRAINT UN_MC_CAT_PROD_DS_CAT UNIQUE (DS_CATEGORIA);
    
-- Adicionando Check constraint para status do chamado (sac)
ALTER TABLE MC_SGV_SAC
    ADD CONSTRAINT CK_MC_SGV_SAC_ST_SAC CHECK (ST_SAC IN ('E', 'C', 'F', 'X'));
    
-- Adicionando Sequence para identificacao do produto
...
--Adicionando Trigger para tabela mc_produto receber o sq_cd_produto
CREATE TRIGGER TRG_MC_PRODUTO_CD_PRODUTO
BEFORE INSERT ON MC_PRODUTO
FOR EACH ROW
WHEN (NEW.CD_PRODUTO IS NULL)
BEGIN
   SELECT SQ_CD_PRODUTO.NEXTVAL
   INTO :NEW.CD_PRODUTO
   FROM dual;
END;

-- Adicionando Sequence para codigo da categoria do produto
CREATE SEQUENCE SQ_CD_CATEGORIA_PROD
    START WITH 1
    INCREMENT BY 1
    MAXVALUE 99999
    NOCACHE
...
--Adicionando Trigger para tabela mc_produto receber o sq_cd_categoria_prod
CREATE TRIGGER TRG_MC_PRODUTO_CD_CAT_PROD
BEFORE INSERT ON MC_PRODUTO
FOR EACH ROW
WHEN (NEW.CD_CATEGORIA IS NULL)
BEGIN
   SELECT SQ_CD_CATEGORIA_PROD.NEXTVAL
   INTO :NEW.CD_CATEGORIA
   FROM dual;
END;

-- Adicionando Sequence para o numero do chamado
...
--Adicionando Trigger para tabela mc_sgv_sac receber o sequence SQ_NUMERO_SAC
CREATE TRIGGER TRG_MC_SGV_SAC_NUM_SAC
BEFORE INSERT ON MC_SGV_SAC
FOR EACH ROW
WHEN (NEW.NR_SAC IS NULL)
BEGIN
   SELECT SQ_NUMERO_SAC.NEXTVAL
   INTO :NEW.NR_SAC
   FROM dual;
END;
